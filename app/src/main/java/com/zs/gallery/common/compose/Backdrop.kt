...
fun rememberBackdropProvider() = remember(::BackdropProvider)

/**
 * Marks the content to be a source of blurred content for the provided [provider].
 *
 * This function registers the content it is applied to as a source that contributes to the blurring
 * effect managed by the given [provider]. Any views or composables marked with [background] using
 * the same [provider] will blur this source content.
 *
 * @param provider The [BackdropProvider] instance that this content will contribute to.
 * @return A [Modifier] that marks the content as a blur source.
 */
fun Modifier.source(provider: BackdropProvider?) = if (provider == null) Modifier else Modifier.hazeSource(provider)
...
/**
 * Applies a hazy effect to the background based on the provided [BackdropProvider].
 *
 ...
 *
 * @param provider The [BackdropProvider] instance that manages the haze effect.
...
) = if (provider == null) Background(Modifier.acrylic(background, accent))
else Background(Modifier.hazeEffect(state = provider) {
